...
const gotoCreate = () => {
  router.push({ path: ROUTES.TEACHER_HOMEWORK_RECOMPOSE_PAPER }).catch(() => {})
}
...
const resumeCompose = async (item: listExaminationItem) => {
  ...
  if (res) {
    ...
    router.push({ path: ROUTES.TEACHER_HOMEWORK_RECOMPOSE_PAPER, query: { assignmentId: encryptedId } }).catch(() => {})
  }
}

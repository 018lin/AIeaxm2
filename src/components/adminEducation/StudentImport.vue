<template>
  <a-modal :open="open" :width="760" centered :footer="null" @cancel="handleClose" class="question-edit-modal">
    ...
    <div class="import-box" v-if="currentState === 0">
      <div class="import-header">批量导入学生</div>
      ...
      <div class="import-item app-surface">
        ...
        <a-timeline>
          ...
          <a-timeline-item style="padding-bottom: 0">
            导入的账号可通过填写的
            <a-tag color="volcano">学号</a-tag>登录，默认密码为
            <a-tag color="volcano">Aa123456</a-tag>
          </a-timeline-item>
        </a-timeline>
      </div>
      ...
      <div class="import-item import-request app-surface">
        ...
        <a-descriptions bordered :column="1" :labelStyle="{ width: '150px' }" class="import-descriptions">
          <a-descriptions-item label="姓名：">填写学生姓名 </a-descriptions-item>
          <a-descriptions-item label="性别：">
            填写学生性别，“<a-tag color="volcano">男</a-tag>”或“<a-tag color="volcano">女</a-tag>”
          </a-descriptions-item>
          <a-descriptions-item label="学号：">填写学生<a-tag color="volcano">学号</a-tag> </a-descriptions-item>
          ...
          <a-descriptions-item label="班级："> 填写学生所在班级 </a-descriptions-item>
        </a-descriptions>
      </div>
    ...
    <div class="import-success" v-else-if="currentState === 1">
      ...
      <div class="success-main">
        ...
        <p class="success-content">
          本次导入{{ successCount }}条学生信息，导入的学生可使用填写的 <b>学号</b>登录，默认密码为
          <b>Aa123456</b>
        </p>
      </div>
    </div>
...
import { downloadStudentTemplate, importStudents } from '@/api/adminEducation/studentIndex'
...
const successCount = ref(0) // 成功导入的学生数量
...
const downloadTemplate = async () => {
  try {
    ...
    if (blob) {
      ...
      link.href = url
      link.download = `学生导入模板_${new Date().getTime()}.xlsx`
      // 触发下载
...
const handleUpload: UploadProps['customRequest'] = async options => {
  ...
  try {
    // 调用上传接口
    const res = await importStudents({ file: file as File })
    if (res.success) {
